...
class Face
{
 public:
	...
	int material;	       // ID del material (index del material)
        ...
        Vector normal;
        void computeNormal(vector<Vertex> &);

        friend QDataStream &operator<<(QDataStream &, const Face &);
        friend QDataStream &operator>>(QDataStream &, Face &);
      };

      QDataStream &operator<<(QDataStream &, const Face &);
      QDataStream &operator>>(QDataStream &, Face &);
