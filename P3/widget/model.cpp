...
QDataStream &operator<<(QDataStream &ds, const Model &m)
{
    ds << m.name;
    ds << m._boundingBox;
    ds << (unsigned int) m.vertices.size();
...
QDataStream &operator>>(QDataStream &ds, Model &m)
{
    unsigned int  n;
    ds >> m.name;
    ds >> m._boundingBox;
...
void Model::Render()// viene del esqueleto
{
...
void Model::RenderSeleccio(GLubyte i)
{
    for(unsigned int cara = 0; cara < faces.size(); cara++)
    {
        glBegin(GL_POLYGON);
