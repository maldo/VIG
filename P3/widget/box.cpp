...
QDataStream &operator<<(QDataStream & ds, const Box &b)
{
    ds << b.minb;
    ds << b.maxb;
...
QDataStream &operator>>(QDataStream &ds, Box &b)
{
    ds >> b.minb;
    ds >> b.maxb;
...
bool Box::intersectaCon(Box & i)
{
    return (minb.x < i.maxb.x && maxb.x > i.minb.x)
            && (minb.y < i.maxb.y && maxb.y > i.minb.y)
            && (minb.z < i.maxb.z && maxb.z > i.minb.z);
}
